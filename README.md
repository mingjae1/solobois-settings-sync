# Share your Antigravity (VS Code compatible) settings with Soloboi's Settings Sync 🚀
...
3. Run `soloboisSettingsSync.shareSettings`.
4. Copy the generated public Gist URL and share it.

## How Sharing Works

- `soloboisSettingsSync.shareSettings` creates a **public** share snapshot Gist.
...
1. Install the extension.
2. Run `soloboisSettingsSync.login`.
3. Run `soloboisSettingsSync.uploadNow` to create/use a Gist, or `soloboisSettingsSync.setGistId` (accepts Gist ID or URL) + `soloboisSettingsSync.downloadNow` to pull from an existing Gist.
...
`soloboisSettingsSync.shareSettings` | Create a public share snapshot and copy its URL (sensitive values are masked).
`soloboisSettingsSync.showHistory` | Open the history of your settings Gist.
`soloboisSettingsSync.setGistId` | Set or clear the target Gist ID manually (Gist URL is also supported).
...
`soloboisSettingsSync.removePrivateExtension` | Remove a registered private/unlisted extension entry.
...
## Antigravity 설정을 공유하세요 (VS Code 호환) with Soloboi's Settings Sync 🚀

Soloboi's Settings Sync는 **Antigravity** 중심으로 설계되었고, **VS Code**와도 호환됩니다. GitHub Gist로 설정을 공유하고 여러 기기에서 동기화할 수 있습니다.

## 공유 먼저 (60초)

1. 익스텐션 설치
2. `soloboisSettingsSync.login` 실행
3. `soloboisSettingsSync.shareSettings` 실행
4. 생성된 공개 Gist URL 복사 후 공유

## 공유 동작 방식

- `soloboisSettingsSync.shareSettings`는 **공개(public)** 공유 스냅샷 Gist를 생성합니다.
...
1. 익스텐션 설치
2. `soloboisSettingsSync.login` 실행
3. 새 Gist로 시작하려면 `soloboisSettingsSync.uploadNow`, 기존 Gist에서 내려받으려면 `soloboisSettingsSync.setGistId`(Gist ID/URL 모두 지원) + `soloboisSettingsSync.downloadNow`
...
`soloboisSettingsSync.shareSettings` | 공유용 공개 스냅샷을 만들고 URL을 복사합니다(민감값은 마스킹).
`soloboisSettingsSync.showHistory` | 설정 Gist의 히스토리를 확인합니다.
`soloboisSettingsSync.setGistId` | 사용할 Gist ID를 수동으로 지정/초기화합니다(Gist URL도 지원).
...
`soloboisSettingsSync.removePrivateExtension` | 등록된 Private/미등록 익스텐션 항목을 제거합니다.
